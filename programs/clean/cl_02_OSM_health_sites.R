...
lookup <- tribble(
  ...
  # 1. OBGYNs / Reproductive / Pediatrics
  "obstetrics|gynecology|ob/gyn|ob-gyn|obgyn|reproductive health|pediatric|pediatrix|neonatal|midwife|midwifery|fertility|maternal|children's|gynaecology|birth",
  "specialists_reproductive",
  ...
  # 3. Cancer Care
  "oncolog|cancer|chemo|chemotherapy|infusion center|radiation center",
  "specialists_cancer",
  ...
  # 6. Specific Medical Specialties
  "allergy|allergist|asthma|audiology|audiologist|hearing|sleep|surgeon|surgical|surgery|cardiology|cardiovascular|dermatolog|urolog|neurolog|endocrinolog|\\bent\\b|ear nose & throat|ear nose and throat|ear\\, nose|gastroenterolog|otolaryngology|surgi-center|digestive|vein care|diabetes|osteoporosis|pulmonary|vascular|anesthesiology",
  "specialists_medical_surgery",
  
  # 7. Generic Doctors
  "doctor|doctors|physician|physicians|specialist|specialists|\\bdr\\.?\\b|\\bdrs\\b|\\bmd\\b|\\bdo\\b|\\bnp\\b|\\bfnp\\b|\\baprn\\b|\\bpa\\b|\\bm\\.d.?\\b|\\bd\\.o.?\\b|\\bpc\\b|\\bpllc\\b|adult medicine",
  "doctors_general",
  ...
  # First Aid / EMS
  "red cross|first aid|\\bems\\b|ambulance|emergency medical service|\\ber\\b|emergency|\\bemt\\b", 
  "first_aid_emergency",
  ...
  "healthcare|health care|health department|health services|health system|health center|wellness center|wellness|medical office|medical offices|medical arts|medical building|medical pavilion|health park|healthsource|aspirus|bickford|health centre|health service|health|phd|ph\\.d", 
  "other_healthcare"
)
...
final_clean = fixed %>% 
  ...
  mutate(amenity = case_when(
    amenity == "first_aid" ~"first_aid_emergency",
...
saveRDS(lookup, "shiny_dashboard/health_sites_lookup_tab.rds")